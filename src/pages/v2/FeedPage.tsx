...
function groupByDate(cards: EnrichedFeedCard[]): DateGroup[] {
  ...
  const groups: Record<string, EnrichedFeedCard[]> = {
    today: [],
    tomorrow: [],
    week: [],
    later: [],
  ...
  if (groups.today.length > 0) {
    result.push({
      key: "today",
      label: `Hoje — ${format(today, "EEE, d MMM", { locale: ptBR })}`,
      cards: groups.today,
    });
  }
  if (groups.tomorrow.length > 0) {
    result.push({
      key: "tomorrow",
      label: `Amanhã — ${format(tomorrow, "EEE, d MMM", { locale: ptBR })}`,
      cards: groups.tomorrow,
    });
  }
  if (groups.week.length > 0) {
    result.push({ key: "week", label: "Próxima semana", cards: groups.week });
  }
  if (groups.later.length > 0) {
    result.push({ key: "later", label: "Mais tarde", cards: groups.later });
  }
...
export default function FeedPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#FAFAF8]">
      {/* Filter chips */}
      <div className="sticky top-0 z-10 bg-[#FAFAF8] px-4 pt-3 pb-2 flex items-center gap-2 overflow-x-auto no-scrollbar">
        {FILTERS.map((f) => {
            ...
            <button
              ...
              className={`shrink-0 h-8 px-3 rounded-full text-xs font-medium transition-colors ${
                active
                  ...
                  : "bg-[#F4F4F1] text-[#6B6B6B]"
              }`}
            >
              {f.label}
              {f.key === "overdue" && overdueCount > 0 && (
                <span className="ml-1 inline-flex items-center justify-center h-4 min-w-[16px] rounded-full bg-destructive text-white text-[10px] px-1">
                  {overdueCount}
                </span>
              )}
            </button>
        ...
        <button
          onClick={() => refetch()}
          className="shrink-0 h-8 w-8 flex items-center justify-center rounded-full bg-[#F4F4F1] text-[#6B6B6B]"
        >
